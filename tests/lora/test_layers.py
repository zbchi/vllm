...
def test_packed_gate_up_dora_scale_stacked_tp2(
    ...
    for tp_rank in range(tp_size):
        ...
        for i, lora_a_i in enumerate(lora_a):
            torch.testing.assert_close(
                lora_linear.lora_a_stacked[i][
                    1, 0, : lora_a_i.shape[0]
                ].float(),
                lora_a_i.float(),
                rtol=1e-3,
                atol=1e-3,
            )
        for i, lora_b_i in enumerate(lora_b):
            shard_size = lora_b_i.shape[0] // tp_size
            start = tp_rank * shard_size
            torch.testing.assert_close(
                lora_linear.lora_b_stacked[i][1, 0, :shard_size].float(),
                lora_b_i[start : start + shard_size].float(),
                rtol=1e-3,
                atol=1e-3,
            )
